...
@php
    ...
    $cfg = $statusConfig[$dataReport->status] ?? $statusConfig['diajukan'];
@endphp

<div class="bg-white rounded-xl shadow p-4 lg:p-6 max-w-3xl mx-auto">

    <a href="{{ route('admin.data_reports.index') }}" class="text-xs text-sky-400 hover:text-sky-600 flex items-center gap-1 mb-3">
        <i class="fas fa-arrow-left"></i> Kembali ke Daftar
    </a>

    <div class="flex items-start justify-between gap-3 mb-4 flex-wrap">
        <div>
            <h1 class="text-lg sm:text-xl font-bold text-gray-800">{{ $dataReport->nama_data }}</h1>
            <p class="text-xs text-gray-400 mt-1">
                Dilaporkan oleh <span class="font-medium text-gray-600">{{ $dataReport->user->name ?? '-' }}</span>
                pada {{ $dataReport->created_at->format('d M Y, H:i') }} WITA
            </p>
        </div>
    ...
    <div class="grid sm:grid-cols-2 gap-4 border-t border-gray-100 pt-4 mb-4">
        <div>
            ...
            <p class="text-sm font-medium text-gray-700">{{ $dataReport->location->nama_wilayah ?? '-' }}</p>
        </div>
        <div>
    ...
    <div class="mb-4">
        <p class="text-xs text-gray-400 mb-1">Deskripsi Kesalahan</p>
        <p class="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{{ $dataReport->deskripsi_kesalahan }}</p>
    </div>

    @if($dataReport->status !== 'diajukan')
        <div class="rounded-lg border border-gray-100 bg-gray-50 p-4 mb-4">
            <p class="text-xs text-gray-400 mb-1">
                Ditinjau oleh <span class="font-medium text-gray-600">{{ $dataReport->reviewer->name ?? '-' }}</span>
                pada {{ $dataReport->reviewed_at?->format('d M Y, H:i') }} WITA
            </p>
            @if($dataReport->admin_notes)
                <p class="text-sm text-gray-700 mt-2">{{ $dataReport->admin_notes }}</p>
            @endif
        </div>
    @endif

    @if(in_array(auth()->user()->group_id, [1, 2], true) && in_array($dataReport->status, ['diajukan', 'ditinjau']))
        <div class="border-t border-gray-100 pt-4">
            <h2 class="text-sm font-semibold text-gray-700 mb-3">Tindak Lanjuti Laporan</h2>
            <form action="{{ route('admin.data_reports.review', $dataReport) }}" method="POST" id="reviewForm">
                @csrf
...
function submitReview(status) {
    ...
    Swal.fire({
        title: `Yakin ingin ${label} laporan ini?`,
        icon: 'question',
